document.addEventListener("DOMContentLoaded", () => {
    ...
    function mostrarProductosPorBusqueda() {
        const terminoBusqueda = obtenerParametroBusqueda();
        if (!terminoBusqueda) return;
        const productosEncontrados = [];
        Object.keys(imagenesPorCategoria).forEach(categoria => {
        ...
        if (productosEncontrados.length > 0) {
            contenedorImagenes.innerHTML = ""; // Limpiar antes de mostrar nuevos resultados
